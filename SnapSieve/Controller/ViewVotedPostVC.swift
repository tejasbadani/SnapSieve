...
class ViewVotedPostVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let gesture1 = UITapGestureRecognizer(target: self, action: #selector(viewUser))
        gesture1.numberOfTapsRequired = 1
        usernameLabel.addGestureRecognizer(gesture1)
        
        let gesture2 = UITapGestureRecognizer(target: self, action: #selector(viewUser))
        gesture2.numberOfTapsRequired = 1
        profileImageView.addGestureRecognizer(gesture2)
        ...
        if let p = post {
            ...
            self.captionLabel.text = p.caption
            let urlProfile = URL(string: p.profileURL)
            ...
            likeButton.fillColor = UIColor.blue
    ...
    @objc func handleScreenshot(){
        ...
        NESTED_BACK = true
        performSegue(withIdentifier: "VotedPostToFeed", sender: nil)
    }
    ...
    @objc func viewUser(sender : UITapGestureRecognizer){
        performSegue(withIdentifier: "viewUserFromVotedPosts", sender: nil)
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "viewUserFromVotedPosts"{
            let viewUser = segue.destination as! ViewUserVC
            viewUser.userName = post.userName
            viewUser.userIDsample = post.userID
            viewUser.profileURL = post.profileURL
